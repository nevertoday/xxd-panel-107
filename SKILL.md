...
6. The source brief's 3:4 canvas is the native `top-bottom` presentation unless the user selects another final size. Regardless of canvas ratio, `top-bottom` always keeps exact 50:50 upper/lower regions, and `left-right` always maps the equal-pair principle to exact 50:50 left/right regions. Only `design-only` and `wallpaper-pack` replace the paired container; every aesthetic, rebus, and transformation instruction remains active.
...
```text
/xxd-panel-107 <source-or-directory> --mode top-bottom,design-only \
...
1. 上下对照（原图在上，107 图像替词诗在下，严格 50:50）
2. 左右对照（原图在左，107 图像替词诗在右，严格 50:50）
...
reality-source visibility, final canvas and device delivery. TOP_BOTTOM keeps
the source brief's native 3:4 unless another final size was explicitly resolved,
and always keeps equal upper/lower regions; LEFT_RIGHT maps the same equal-pair
rule to two exact left/right regions. DESIGN_ONLY and
WALLPAPER_PACK replace the old paired container.
...
Create one complete canvas (native ratio 3:4 unless another final canvas was
explicitly resolved) with exactly two horizontal regions of equal height.
The REALITY VIEW occupies the upper 50%; the TRANSFORMED DESIGN occupies
the lower 50%. The boundary is exactly at the vertical midpoint. Do not create
a third band, footer, shared title strip, border, or transitional region. Keep
all rebus typography and image words inside the lower 50%.
```
...
Create one complete canvas with exactly two vertical regions of equal width.
The REALITY VIEW occupies the left 50%; the TRANSFORMED DESIGN occupies the
right 50%. The boundary is exactly at the horizontal midpoint and both regions
run from the top edge to the bottom edge. Do not create a third band, footer,
shared title strip, border, or transitional region. Keep all rebus typography
and image words inside the right 50%.
```
...
Use **one complete-canvas generation per output**. Give the image model the original source exactly once, the complete source-brief body, common preamble, one selected mode block, one text block and final canvas in one request. Never feed a generated poster, transformed intermediate, sample, or pre-styled derivative back as the source; never generate the lower design and then run the assembled poster through a second style pass. The model decides how to preserve the important subject and context inside the selected mode contract. Retry a failed complete canvas once by restating only a meaningful failed content or delivery constraint. Use `scripts/compose_panel.py` only as a final lossless assembly/audit fallback when the required 50:50 geometry or pixel-identical source cannot otherwise be delivered. The script may crop/paste/size/audit raster files; it must never invent or restyle artwork, and its output must never become a new generation source.
...
```text
~/Desktop/xxd/xxd-panel-107/<fresh-task>/
...
- `top-bottom` uses the resolved whole-canvas size (native ratio 3:4) with exactly two equal-height horizontal regions, reality above and transformed design below; the midpoint is 50:50 and there is no third band;
- `left-right` has exactly two equal-width vertical regions from top edge to bottom edge, reality left and transformed design right; the midpoint is 50:50 and typography never becomes a shared third band;
...
- exact paired geometry is verified for every `top-bottom` and `left-right` asset; deterministic assembly remains a lossless last resort, never a second visual treatment;
- the transformed region contains one grammatically ordered rebus sentence: printed text fragments and source-derived image words alternate along a clear reading path; each pictured word occupies the missing word's position and is essential to reconstructing the sentence;
- a replaced word never appears again as printed text; reject a complete printed sentence with decorative pictures, loose illustrations beside copy, sticker walls, card lists, regular grids, or disconnected collages;
...
- `references/xxd-panel-107-prompt.zh-CN.md`
- `references/xxd-panel-107-prompt.en.md`
