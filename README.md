...
```text
读取照片的情绪与关系 → 写出一条完整短句 → 选出适合视觉化的词 → 删除这些文字 → 用源自原图的手绘图像词原位替换 → 沿“文字→图像词→文字”路径完成可读视觉诗
```

## 成品中最容易识别的风格特点

- 上下模式原生为 3:4，也可使用用户选择的其他整画布尺寸，但始终严格 50:50；左右模式同样严格 50:50，绝不增加第三条底栏。
- 下方或右方先有完整句意，再把若干词删去并以图像原位替代；被替换词不会在画面里重复印出。
- 每个图像词都来自当前照片，却统一重绘成简化轮廓、平面色块与轻微手工感的现代编辑插画，而不是原图截图。
- 图像词色块可方、窄、长或不规则，并按语义改变大小，但始终嵌在共享基线与阅读路径中，不形成卡片墙。
- 2–4 行文字、图像词、留白与轻微错位共同建立清晰节奏；明亮柔和的色彩从原图最有生命力的颜色重新调制。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-107-prompt.en.md)
